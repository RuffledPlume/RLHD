...
final class JobHandle extends AbstractQueuedSynchronizer {
	...
	private static final ThreadLocal<LinkedBlockingDeque<JobHandle>> CYCLE_STACK = ThreadLocal.withInitial(LinkedBlockingDeque::new);
	...
	synchronized void setCompleted() {
		...
		while((dep = dependants.pollFirst()) != null) {
			...
			if (dep.isIdle() && dep.depCount.decrementAndGet() == 0) {
				...
				// Decide if we should store the job within the local or main work queues
				BlockingDeque<JobHandle> queue = worker.localWorkQueue.size() < 10 ? worker.localWorkQueue : JobSystem.INSTANCE.workQueue;
