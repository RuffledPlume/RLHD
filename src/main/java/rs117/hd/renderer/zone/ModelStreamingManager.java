...
public class ModelStreamingManager {
	...
	private void uploadTempModel(
		...
		if (culledFaces.length > 0 &&
			...
			final DynamicModelVAO.VAOView shadowView = ctx.beginDraw(VAO_SHADOW, culledFaces.length);
		...
		if (visibleFaces.length > 0) {
			...
			final DynamicModelVAO.VAOView opaqueView = ctx.beginDraw(renderable instanceof Player ? VAO_PLAYER : VAO_OPAQUE, opaqueFaceCount);
			final DynamicModelVAO.VAOView alphaView = alphaFaceCount > 0 ? ctx.beginDraw(VAO_ALPHA, alphaFaceCount) : opaqueView;
	...
	private void uploadDynamicModel(
		...
		try {
			...
			if (visibleFaces.length > 0) {
				...
				final DynamicModelVAO.VAOView opaqueView = ctx.beginDraw(VAO_OPAQUE, opaqueFaceCount);
				final DynamicModelVAO.VAOView alphaView = alphaFaceCount > 0 ? ctx.beginDraw(VAO_ALPHA, alphaFaceCount) : opaqueView;
