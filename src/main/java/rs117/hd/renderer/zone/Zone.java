...
public class Zone {
	...
	public void setMetadata(WorldViewContext viewContext, SceneContext sceneContext, int mx, int mz) {
		...
		baseX = (mx - (sceneContext.sceneOffset >> 3)) << 10;
		baseZ = (mz - (sceneContext.sceneOffset >> 3)) << 10;
				...
				for (int z = m.lz >> 3; z <= m.uz >> 3; ++z) {
					...
					if (distance < max) {
						...
						if (zx2 >= 0 && zx2 < zones.length && zz2 >= 0 && zz2 < zones[0].length) {
							...
							if(z2.inSceneFrustum && z2.initialized && (z2.occlusionQuery == null || !z2.occlusionQuery.isFullyOccluded())) {
								max = distance;
