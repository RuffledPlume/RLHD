...
public class Zone {
	...
	synchronized void addTempAlphaModel(ModelOverride modelOverride, DynamicModelVAO.VAOView view, int level, int x, int y, int z) {
	...
	synchronized void addPlayerModel(DynamicModelVAO.VAOView view, int level, int x, int y, int z) {
		AlphaModel m = modelCache.poll();
		if (m == null)
			m = new AlphaModel();
		m.id = -1;
